...
> **版本:** v1.0.0 | **更新:** 2026-03-25 | **版本控制:** 本文件由 Claude Code 自動維護

---

## 版本控制規則

- 本文件每完成一個 Sprint 後由 Claude Code 自動更新
- ✅ = 已完成 | ⬜ = 待開發 | 🔄 = 進行中 | ❌ = 取消
...
- ⬜ GitHub repo + 完整目錄結構 (所有 `__init__.py`)
...
**驗收標準:** `python -m promptbim --version` 可執行; `python -m promptbim gui` 跑出空白 Qt 視窗
...
- ⬜ 測試: 拖放 .geojson → 顯示土地輪廓 + 面積 + 退縮線

**驗收標準:** 拖放 sample_parcel.geojson 進視窗 → 顯示土地輪廓 + 面積 + 退縮線
...
**驗收標準:** `python examples/01_simple_box.py` → 生成 .ifc + .usda 兩個檔案都可開啟
