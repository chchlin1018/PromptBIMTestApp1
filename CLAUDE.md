...
1. 讀取 PROMPT_P{X}.md + 必讀文件
2. 執行環境檢查腳本
3. **★ 發送啟動 iMessage ★**（必須在任何 Task 之前）

### 執行階段
4. 依序執行 Task / Part
5. 每完成一個 Part → **發送 Part 完成 iMessage**

### 收尾階段
6. xcodebuild 驗證 ✅
7. pytest 驗證 ✅
8. Xcode pbxproj 完整性檢查 ✅
9. 全量文件同步 ✅
10. Git Commit + Push ✅
11. 建立下一個 PROMPT（通過合規性檢查）✅
12. 輸出 Sprint 完成報告（依模板）✅
13. **★ 發送最終完成 iMessage ★**
...
*CLAUDE.md v1.10.0 | 2026-03-26 | 變更: 新增 [MANDATORY] Sprint 啟動通知必須是第一個動作 + 完整 Sprint 執行流程（開始→執行→收尾 13 步）+ 合規性檢查加入啟動通知步驟檢查 + 常見遺漏加入 P17 啟動未通知案例*
