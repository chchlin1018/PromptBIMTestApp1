...
### Step 7: iMessage 通知 Michael ✅

> ⚠️ **每次工作結束時（無論成功或失敗），必須透過 iMessage 發送通知。**
> 這讓 Michael 可以在 iPhone 上即時收到 Mac Mini 的開發進度。
...
./scripts/notify_imessage.sh "✅ Sprint ${SPRINT} 完成
🧪 pytest: ${TEST_COUNT}
...
**Sprint 失敗或中斷時：**
```bash
./scripts/notify_imessage.sh "❌ Sprint ${SPRINT} 失敗
❗ ${ERROR_DESCRIPTION}
🧪 pytest: ${TEST_RESULT}
⚠️ 需要人工介入"
```
...
osascript -e 'tell application "Messages"
    ...
    send "🏗️ PromptBIM — Sprint 完成通知" to targetBuddy
end tell'
```

**注意：**
- 通知失敗不影響 Sprint 成功/失敗判定（best-effort）
- 如果 osascript 失敗（Messages 未登入），記錄在 log 中繼續執行
- Messages.app 必須已登入 iCloud（chchlin1018@icloud.com）
...
*CLAUDE.md v1.4.0 | 2026-03-25 | 新增: Step 7 iMessage 通知（Sprint 完成/失敗自動發送到 iPhone）*
