...
如果 `notify` 不可用，使用 fallback：
```bash
./scripts/notify_imessage.sh "..." || echo "..." > /tmp/imessage_notify.txt
```
...
> ⚠️ **啟動通知必須是第一個動作（見上方獨立規則）。**
> ⚠️ **如果 Sprint 包含多個 Part，每完成一個 Part 也必須發送通知。**
> ⚠️ **如果 Sprint 執行中遇到錯誤或中斷，也必須發送通知（見下方錯誤通知）。**
...
### ⚠️ 每次發送通知時，必須同時在 terminal 印出相同訊息

> **所有 notify 呼叫都必須搭配 echo，讓訊息同步出現在 terminal 上。**

```bash
# ✅ 正確做法：notify + echo 同步
MSG="🏗️ PromptBIM
✅ Sprint P17 — Part A 完成
📋 CI/CD 修復
🧪 pytest: 740
📍 $(hostname -s) | $(date '+%m/%d %H:%M')"
echo "$MSG"
...
```bash
# ❌ 錯誤做法：只有 notify，terminal 沒有輸出
notify "Sprint P17 — Part A 完成"
```

**規則：先 `echo`，再 `notify`。確保 terminal 和 iMessage 都能看到訊息。**

### Part 完成時通知（每個 Part 完成後必須發送）

```bash
MSG="🏗️ PromptBIM
✅ Sprint ${SPRINT} — Part ${PART} 完成
📋 ${PART_NAME}
🧪 pytest: ${TEST_COUNT}
📍 $(hostname -s) | $(date '+%m/%d %H:%M')"
echo "$MSG"
...
> ⚠️ **如果 Sprint 執行過程中發生無法自行修復的錯誤，或因任何原因需要中斷執行，**
> ⚠️ **必須在中斷前發送 iMessage 通知，說明錯誤原因和當前進度。**
...
| Xcode signing 問題 | 缺少的 certificate + 建議操作 |
| pytest 無法通過（嘗試修復 3 次後仍失敗） | 失敗的測試名稱 + 錯誤訊息摘要 |
| xcodebuild 失敗（嘗試修復 3 次後仍失敗） | build error 摘要 |
| 依賴安裝失敗 | 套件名 + 錯誤訊息 |
...
- 嘗試自行修復（最多 3 次）後仍無法解決 → **必須通知**
- 通知後可以繼續嘗試，但用戶已被告知問題
- **即使 Claude Code 即將因 context 限制結束，也必須在結束前發送通知**
...
✅ iMessage 通知已發送（啟動 + 每個 Part 完成 + 最終完成）
...
| P17 啟動未發送 iMessage | 用戶不知道已開始 | **啟動通知是第一個動作** |
| Sprint 中斷但未通知 | 用戶不知道出了問題 | **錯誤/中斷必須通知** |
...
1. 讀取 PROMPT_P{X}.md + 必讀文件
2. 執行環境檢查腳本
3. **★ 發送啟動 iMessage（echo + notify）★**（必須在任何 Task 之前）
...
10. 全量文件同步 ✅
11. Git Commit + Push ✅
12. 建立下一個 PROMPT（通過合規性檢查）✅
13. 輸出 Sprint 完成報告（依模板）✅
14. **★ echo + notify 最終完成 iMessage ★**
...
□ Sprint 完成報告已按模板輸出
□ iMessage 通知已發送（啟動 + 每個 Part 完成 + 最終完成）
□ 所有通知都有在 terminal 印出（echo + notify）
```
...
*CLAUDE.md v1.11.0 | 2026-03-26 | 變更: 新增 [MANDATORY] 錯誤/中斷時必須發送 notify 通知（含原因描述）+ 所有 notify 必須同步 echo 到 terminal*
