...
_rule(
    PlacementRule(
        "pm25_sensor", "per_space", 1.0, min_per_space=1, max_per_space=1, min_area_sqm=30
    )
)
_rule(PlacementRule("light_sensor", "per_sqm", 0.015, min_per_space=1, max_per_space=4))
_rule(
    PlacementRule(
        "noise_sensor", "per_space", 1.0, min_per_space=1, max_per_space=1, min_area_sqm=40
    )
)
_rule(
    PlacementRule("voc_sensor", "per_space", 1.0, min_per_space=0, max_per_space=1, min_area_sqm=50)
)
...
_rule(
    PlacementRule(
        "vav_controller", "per_sqm", 0.01, min_per_space=1, max_per_space=4, min_area_sqm=20
    )
)
...
_rule(
    PlacementRule(
        "air_purifier_ctrl", "per_space", 1.0, min_per_space=0, max_per_space=1, min_area_sqm=30
    )
)
...
class RulesEngine:
    ...
    def compute_count(
        ...
        *,
        ...
        elif mode == "per_floor" or mode == "per_building":
        ...
        logger.debug(
            "Rule %s (mode=%s, area=%.1f): count=%d", monitor_type_id, mode, area_sqm, result
        )
        return result
