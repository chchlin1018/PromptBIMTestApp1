...
def generate_factory_plan(
    ...
    hall_footprint = [
        (ox, oy),
        (ox + hall_width, oy),
        (ox + hall_width, oy + total_depth),
        (ox, oy + total_depth),
    ]
    office_footprint = [
        (ox + hall_width, oy),
        (ox + total_width, oy),
        (ox + total_width, oy + total_depth),
        (ox + hall_width, oy + total_depth),
    ]
    full_footprint = [
        (ox, oy),
    ...
    ext_walls = [
        ...
        WallDef(
            start=(ox + total_width, oy),
            end=(ox + total_width, oy + total_depth),
            thickness_m=0.3,
            wall_type="exterior",
        ),
        WallDef(
            start=(ox + total_width, oy + total_depth),
            end=(ox, oy + total_depth),
            thickness_m=0.3,
            wall_type="exterior",
        ),
        ...
        spaces.append(
            SpaceDef(
                ...
                boundary=[
                    (ox + hall_width, ry),
                    (ox + total_width, ry),
                    (ox + total_width, ry + office_room_depth),
                    (ox + hall_width, ry + office_room_depth),
                ],
    ...
    for r in range(office_rooms):
        openings.append(
            OpeningDef(
                wall_index=1,
                offset_m=r * office_room_depth + 2.0,
                width_m=2.0,
                height_m=1.5,
                sill_height_m=0.9,
                opening_type="window",
    ...
    if num_stories >= 2:
        ...
        ext2 = [
            ...
            WallDef(
                start=(ox + hall_width, oy + total_depth),
                end=(ox + hall_width, oy),
                wall_type="exterior",
            ),
        ]
        walls2.extend(ext2)
