...
class BCRRule(BaseRule):
    ...
    def check(self, plan, land, zoning) -> list[CheckResult]:
        ...
        logger.debug(
            "BCRRule: actual_bcr=%.4f, limit=%.4f → %s",
            actual_bcr,
            limit,
...
class FARRule(BaseRule):
    ...
    def check(self, plan, land, zoning) -> list[CheckResult]:
        ...
        logger.debug(
            "FARRule: actual_far=%.4f, limit=%.4f → %s",
            actual_far,
            limit,
...
class HeightLimitRule(BaseRule):
    ...
    def check(self, plan, land, zoning) -> list[CheckResult]:
        ...
        logger.debug(
            "HeightLimitRule: height=%.2fm, limit=%.2fm → %s",
            height,
            limit,
...
class CeilingHeightRule(BaseRule):
    ...
    def check(self, plan, land, zoning) -> list[CheckResult]:
        ...
        for story in plan.stories:
            ...
            logger.debug(
                "CeilingHeightRule: %s net_height=%.2fm (h=%.2f - slab=%.2f)",
                story.name,
                net_height,
                story.height_m,
                story.slab_thickness_m,
            )
            if net_height < 2.1:
                results.append(
                    self._fail(
...
class ParkingRule(BaseRule):
    ...
    def check(self, plan, land, zoning) -> list[CheckResult]:
        ...
        logger.debug(
            "ParkingRule: total_area=%.1f, zone=%s, threshold=%d, required=%d",
            total_area,
            zone,
            threshold,
            required_spaces,
