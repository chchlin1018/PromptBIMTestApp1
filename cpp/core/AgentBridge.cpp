...
std::string AgentBridge::executeJson(const std::string& requestJson) {
    try {
        ...
        else if (actionStr == "move")           { req.action = AgentAction::MoveEntity; req.entityId = j.value("id", ""); auto p = j["position"]; req.vec = {p[0], p[1], p[2]}; }
        else if (actionStr == "rotate")         { req.action = AgentAction::RotateEntity; req.entityId = j.value("id", ""); auto r = j["rotation"]; req.vec = {r[0], r[1], r[2]}; }
        else if (actionStr == "resize")         { req.action = AgentAction::ResizeEntity; req.entityId = j.value("id", ""); auto d = j["dimensions"]; req.vec = {d[0], d[1], d[2]}; }
        else if (actionStr == "add")            { req.action = AgentAction::AddEntity; req.entityId = j.value("id", ""); req.entityType = entityTypeFromString(j.value("type", "Generic")); req.queryString = j.value("name", ""); auto p = j["position"]; req.vec = {p[0], p[1], p[2]}; }
